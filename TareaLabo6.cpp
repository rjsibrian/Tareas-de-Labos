...
void HojaCalc::sumarCol(int col){ // Función que sumará la columna que reciba desde main, en este caso la columna 2
     nodo *s = pHE;
   do{
       if(s->nCol == col){
           int sum = 0;
           ...
           cout <<"La suma de la columna "<<col<<" es igual a: "<< sum << endl;
           break;
       }
       s =s->sigCol;
   }while(s->nCol != 0);
...
int main(void)
{
    ...
    // la mariz se imprime por filas:
    hojaCalculo.imprimirHojaElec();
    hojaCalculo.sumarCol(2); // Le decimos a la función que nos sume la columna 2
